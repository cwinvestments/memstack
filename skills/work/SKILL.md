...
2. For each task, save to SQLite:
   ```bash
   python ~/Projects/memstack/db/memstack-db.py add-plan-task '{"project":"<name>","task_number":<n>,"description":"<task>","status":"pending"}'
   ```
...
2. Update individual task statuses:
   ```bash
   python ~/Projects/memstack/db/memstack-db.py update-task '{"project":"<name>","task_number":<n>,"status":"completed"}'
   ```
...
- Fallback: `~/Projects/memstack/memory/projects/` (legacy markdown)
