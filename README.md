...
[![Version: 3.10.0](https://img.shields.io/badge/Version-3.10.0-green.svg)](CHANGELOG.md)
...
```
This unlocks the 44 Pro-exclusive skills (87 free + 44 Pro = 131 total). Free-tier users can skip this step: type `list skills` to verify the 87 free skills loaded.
...
| Tier | Skills | Access |
|------|--------|--------|
| **Free** | 87 skills | Included with MemStack™ base |
| **Pro** | 131 total (87 free + 44 Pro-exclusive) | Requires Pro Skill Loader ([memstack.pro](https://memstack.pro)) |
...
- **On-demand loading**: Skills load from the catalog only when matched, no context bloat from 131 skills
...
MemStack ships a small set of hooks in `hooks/hooks.json`. They register themselves when the plugin is installed and need no configuration.
...
Every hook that can refuse an action has a kill switch. Set the variable to `1` in your environment and that hook skips itself entirely: it reads nothing, decides nothing, and prints one line to stderr saying it was skipped.
