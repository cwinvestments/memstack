...
## Protocol
1. Run: `python ~/Projects/memstack/skills/echo/search.py "<keywords>" --top-k 5` for semantic vector search
2. Run: `python ~/Projects/memstack/db/memstack-db.py search "<keywords>"` (add `--project <name>` if project is known)
3. Run: `python ~/Projects/memstack/db/memstack-db.py get-sessions <project> --limit 5` for recent sessions
4. Run: `python ~/Projects/memstack/db/memstack-db.py get-insights <project>` for decisions and patterns
...
7. If all sources return nothing, check `~/Projects/memstack/memory/sessions/` as fallback
