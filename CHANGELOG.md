...
## v3.10.0 (unreleased) - Two standing rules become gates that customers receive
...
### Notes
- **MINOR, not patch: this adds customer-facing behaviour that can refuse a tool call.** No skill is added, removed, renamed or recategorized, and the skill count is unchanged at 87 free and 131 total.
...
- **Before publishing this release, delete the two guard entries from `.claude/settings.json`.** This repository registers the guards itself because its own installed plugin is pinned to the published version and would otherwise leave the plugin's source tree ungated. Exactly one registration fires today. Once the plugin cache carries these hooks, this repository alone would fire both.
